<template>
    <div class="bg-white rounded-t-2xl md:rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto md:m-4 md:max-h-[85vh]">
        ...
        <div class="bg-gradient-to-r from-green-700 to-green-500 rounded-lg flex flex-col md:flex-row md:items-center justify-between p-4 md:p-6 sticky top-0 z-10 gap-3 md:gap-8">
            <h2 class="text-xl md:text-3xl font-semibold text-white">Risks & Limitations</h2>
                ...
                >
                    ...
                    <div class="flex-1">
                        <p class="text-base md:text-lg text-gray-900 leading-relaxed">
                            <b class="text-gray-900">{{ risk.risk }}</b> — 
                            <span class="text-gray-700">{{ risk.data_basis }}</span>
                            <span v-if="risk.severity" class="text-gray-600 text-sm md:text-base"> (Severity: <b>{{ risk.severity }}</b>)</span>
                        </p>
                    </div>
                </li>
            </ul>
